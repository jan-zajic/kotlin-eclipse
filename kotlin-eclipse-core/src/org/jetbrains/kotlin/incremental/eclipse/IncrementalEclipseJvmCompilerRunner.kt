...
package org.jetbrains.kotlin.incremental.eclipse
...
import org.jetbrains.kotlin.incremental.commonCacheVersions
...
fun makeEclipseIncrementally(
    ...
    withIC {
        ...
        compiler.compile(kotlinFiles, args, messageCollector) {
			val c = InputsCache::class.java
			val field = c.declaredFields.find { it.name == "sourceSnapshotMap" }
			field?.isAccessible = true
			val map = field?.get(it.inputsCache) as FileSnapshotMap
			map.compareAndUpdate(sourceFiles)
        }
    }
}
...
class IncrementalJvmCompilerRunner(
    ...
    override fun calculateSourcesToCompile(caches: IncrementalJvmCachesManager, changedFiles: ChangedFiles.Known, args: K2JVMCompilerArguments): CompilationMode {
        ...
        val lastBuildInfo = EclipseBuildInfo.read(lastBuildInfoFile)
    ...
    private fun getClasspathChanges(
            ...
            lastBuildInfo: EclipseBuildInfo?
    ): ChangesEither {
